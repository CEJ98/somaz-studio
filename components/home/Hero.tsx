...
export default function Hero() {
  ...
  const contentOpacity = useTransform(scrollYProgress, [0, 0.7, 1], reduced ? [1, 1, 1] : [1, 1, 0])
  const titleY = useTransform(scrollYProgress, [0, 1], reduced ? ['0%', '0%'] : ['0%', '20%'])
  const scrollIndicatorOpacity = useTransform(scrollYProgress, [0, 0.15], reduced ? [1, 1] : [1, 0])

  return (
    <section
      ref={sectionRef}
      className="relative"
      style={{ height: reduced ? '100svh' : '220vh' }}
    >
      <div className="sticky top-0 h-[100svh] min-h-[700px] overflow-hidden flex flex-col justify-end">
        <div className="absolute inset-0">
          ...
          />
          {!reduced && (
            <HeroScrollCanvas targetRef={sectionRef} className="absolute inset-0" />
          )}
        </div>
