  ...
  return (
    <>
      ...
      <section className="noise-overlay relative min-h-screen flex flex-col justify-center px-6 md:px-10 pt-24 pb-20">
        ...
        >
          ...
          <motion.div
            ...
            transition={{
              height: { delay: 1.5, duration: 1, ease },
      ...
      <section className="px-6 md:px-10 py-24 md:py-32 border-t border-border">
        <div className="max-w-7xl mx-auto">
          <AnimatedSection className="mb-16">
            <p className="font-sans text-xs tracking-widest uppercase text-accent mb-3">How We Work</p>
            <h2 className="font-serif font-semibold text-foreground" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)' }}>
            ...
            {[
              {
                n: '01',
                title: 'Brief',
                desc: 'We listen. Understand your vision, constraints, and timeline before touching anything.',
              },
              {
                n: '02',
                title: 'Concept',
                desc: 'We develop a tailored design direction — spatial, visual, material. Refined until it feels inevitable.',
              },
              {
                n: '03',
                title: 'Deliver',
                desc: 'Polished output, on schedule. Ready to present, sell, permit, or build.',
              },
            ].map((step, i) => (
              <motion.div
                key={step.n}
                ...
                viewport={{ once: true, margin: '-60px' }}
                transition={{ duration: 0.7, delay: i * 0.12, ease: ease }}
                className="border-t border-border pt-8 pb-10 md:pr-16 relative"
              >
                <span
                  className="absolute top-4 right-0 font-serif font-bold text-foreground select-none pointer-events-none opacity-[0.03]"
                  style={{ fontSize: '8rem', lineHeight: 1 }}
                  aria-hidden="true"
                >
                  {step.n}
                </span>
                <p className="font-sans text-xs tracking-widest uppercase text-accent mb-4">{step.n}</p>
                <h3 className="font-serif text-2xl font-semibold text-foreground mb-3">{step.title}</h3>
                <p className="font-sans font-light text-foreground/60 leading-relaxed">{step.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-8 md:gap-0 items-center">
          ...
          <AnimatedSection>
            <p className="font-sans font-light text-foreground/60 text-lg leading-relaxed mb-8">
              Somaz Studio brings together design sensibility and cutting-edge visualization tools to transform spaces
              across Miami, Latin America, and beyond.
            </p>
            <Link
              href="/about"
