...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: "Servicios | Somaz Studio",
    description: "Servicios integrales de arquitectura y diseño.",
...
const processSteps = [
  { step: "01", title: "Diagnóstico", description: "Escuchamos tu visión, analizamos el lugar y definimos los objetivos del proyecto con toda claridad." },
  { step: "02", title: "Concepto", description: "Desarrollamos la idea generadora, explorando múltiples posibilidades antes de decidir la dirección." },
  { step: "03", title: "Diseño", description: "Refinamos el concepto en un proyecto ejecutivo completo: planos, especificaciones y presupuesto detallado." },
  { step: "04", title: "Construcción", description: "Supervisamos cada etapa del proceso constructivo para garantizar que la obra refleje fielmente el diseño." },
  { step: "05", title: "Entrega", description: "Acompañamos la entrega y el inicio de vida del espacio, asegurándonos de que todo funcione a la perfección." },
];
...
export default function ServicesPage() {
  return (
    <>
      ...
      {/* Mobile: accordion */}
      <div className="md:hidden px-6 pb-8 bg-paper">
        <ServicesAccordion services={services} />
      </div>

      {/* Desktop: full detail */}
      <section className="hidden md:block py-8 px-12 bg-paper">
        <div className="max-w-[1400px] mx-auto">
          {services.map((service, i) => (
            <FadeIn key={service.number} delay={i * 0.05}>
              <div
                id={service.title.toLowerCase().replace(/\s+/g, "-").replace(/[áéíóúñ]/g, (c) => ({á:"a",é:"e",í:"i",ó:"o",ú:"u",ñ:"n"}[c] ?? c))}
                className={`py-16 md:py-20 border-b border-border ${
                  i % 2 === 1 ? "bg-cream -mx-12 px-12" : ""
                }`}
              >
                <div className="grid grid-cols-12 gap-10 md:gap-16">
                  <div className="col-span-4">
                    <span className="section-label text-gold text-lg mb-4 block">{service.number}</span>
            ...
            {processSteps.map((step, i) => (
              <FadeIn key={step.step} delay={i * 0.1}>
                <div className="py-8 md:py-0 border-b md:border-b-0 md:border-r border-paper/10 last:border-0 md:pr-6">
                  ...
                  <h3 className="font-heading text-xl text-paper font-light mb-3">{step.title}</h3>
                  <p className="font-body text-paper/50 text-sm leading-relaxed">{step.description}</p>
                </div>
              </FadeIn>
            ))}
          </div>
